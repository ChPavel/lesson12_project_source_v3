...
show_foto_blueprint = Blueprint('show_foto_blueprint', __name__, template_folder=templates)
...
@show_foto_blueprint.route('/search')
def search_page():
    ...
    if s is None or s == "":
        posts = data_manager.get_oll()
    else:
        posts = data_manager.search(s)

    return render_template("post_list.html", posts=posts, s=s)

@show_foto_blueprint.errorhandler(DataSourceBrokenException)
